(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".certificates-section__swiper")) new swiper_core_Swiper(".certificates-section__swiper", {
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            speed: 800,
    ...
    function initCart() {
        ...
        if (cartButton && cart) document.addEventListener("click", (e => {
            if (e.target.closest(".header__cart-btn")) openCart(); else if (!e.target.closest(".cart__body") || e.target.closest(".cart__close-btn")) closeCart();
        }));
    }
    function initCustomSelect(container) {
        ...
        document.querySelectorAll(container + ".custom-select").forEach((function(select) {
            ...
            select.querySelectorAll("option").forEach((function(option) {
                if (!option.hasAttribute("disabled")) template += '<span class="custom-option ' + option.className + '" data-value="' + option.value + '">' + option.innerHTML + "</span>";
            }));
            template += "</div></div>";
